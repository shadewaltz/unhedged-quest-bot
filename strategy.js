...
export class Strategy {
  ...
  selectOutcome(market, stats, currentPrice) {
    ...
    const reason = `Majority: ${(majorityConfidence*100).toFixed(0)}% on ${outcomes[majorityIndex].label} | ${priceSignal} | Combined: ${(combinedScore*100).toFixed(0)}%`;
  ...
  parsePriceTarget(question) {
    ...
    const isBelow = question.toLowerCase().includes('below');
