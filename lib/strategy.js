export class Strategy {
  ...
  selectOutcome(market, stats, currentPrice, amount = 0) {
    ...
    const expectedProfit = betAmount * (estimatedPayout - 1);

    if (minPayoutThreshold > 0 && expectedProfit < minPayoutThreshold) {
      return {
        shouldBet: false,
        reason: `Payout too low: ${expectedProfit.toFixed(2)} CC (need >=${minPayoutThreshold.toFixed(2)} CC)`,
        index: finalIndex,
        confidence: 0
      };
    }

    const reason = `Majority: ${(majorityConfidence * 100).toFixed(0)}% on ${outcomes[majorityIndex].label} | ${priceSignal} | Payout: ${expectedProfit.toFixed(2)} CC (${estimatedPayout.toFixed(2)}x) | Combined: ${(combinedScore * 100).toFixed(0)}%`;
