export class Strategy {
  ...
  decide(market, stats, currentPrice, availableBalance, minBet) {
    ...
    const outcome = this.selectOutcome(market, stats, currentPrice);
  ...
  selectOutcome(market, stats, currentPrice) {
    ...
    // Signal 3: Payout check
    const finalProb = finalIndex === 0 ? prob0 : prob1;
    const estimatedPayout = finalProb > 0 ? 1 / finalProb : 0;
    const minPayoutThreshold = this.config.minPayoutThreshold || 0;

    if (minPayoutThreshold > 0 && estimatedPayout < minPayoutThreshold) {
      return {
        shouldBet: false,
        reason: `Payout too low: ${estimatedPayout.toFixed(2)}x (need >=${minPayoutThreshold.toFixed(2)}x)`,
        index: finalIndex,
        confidence: 0
      };
    }

    const reason = `Majority: ${(majorityConfidence * 100).toFixed(0)}% on ${outcomes[majorityIndex].label} | ${priceSignal} | Payout: ${estimatedPayout.toFixed(2)}x | Combined: ${(combinedScore * 100).toFixed(0)}%`;
